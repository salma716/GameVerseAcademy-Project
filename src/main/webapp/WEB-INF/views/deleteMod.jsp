...
<head>
    ...
    <title>GameVerse Academy — Supprimer un Mod</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ph .ey {
            font-size:.62rem; letter-spacing:.25em; text-transform:uppercase; color:var(--rose);
        ...
        .ph h1 {
            ...
            background:linear-gradient(135deg,#fff 0%,#f9a8d4 55%,var(--rose) 100%);
        ...
        /* CONFIRM CARD */
        .confirm-card {
            background:var(--surface); border:1px solid rgba(244,114,182,.2); border-radius:16px;
            padding:2.5rem; max-width:560px; position:relative; overflow:hidden;
        ...
        .warn-icon {
            ...
            display:flex; align-items:center; justify-content:center;
            margin-bottom:1.5rem; box-shadow:0 0 20px rgba(244,114,182,.2);
        }
        ...
        .btn-confirm-delete {
            ...
            color:#fce7f3; font-family:'Cinzel',serif;
            font-size:.75rem; font-weight:600; letter-spacing:.1em; text-transform:uppercase;
            padding:.75rem 1.5rem; cursor:pointer;
            box-shadow:0 0 20px rgba(244,114,182,.25), inset 0 1px 0 rgba(255,255,255,.08);
            transition:all .25s; text-decoration:none;
        }
        .btn-confirm-delete:hover { background:linear-gradient(135deg,#be185d,#ec4899); transform:translateY(-2px); box-shadow:0 0 32px rgba(244,114,182,.4); }
        .btn-cancel {
...
<main class="main">
    ...
    <div class="confirm-card">
        ...
        <p class="warn-text">
            Vous êtes sur le point de supprimer définitivement le mod
            <strong>« <%= mod.getTitle() %> »</strong>.
        ...
        <div class="mod-info">
            <div class="mod-info-row">
                ...
                <span class="mod-info-value">#<%= String.format("%02d", mod.getId()) %></span>
            </div>
            <div class="mod-info-row">
                <span class="mod-info-label">Titre</span>
                <span class="mod-info-value"><%= mod.getTitle() %></span>
            </div>
            <div class="mod-info-row">
                <span class="mod-info-label">Catégorie</span>
                <span class="mod-info-value"><%= mod.getCategory() != null ? mod.getCategory() : "—" %></span>
            </div>
            <div class="mod-info-row">
                <span class="mod-info-label">Auteur</span>
                <span class="mod-info-value"><%= mod.getAuthor() != null ? mod.getAuthor() : "—" %></span>
            </div>
        </div>

        <div class="confirm-actions">
            <!-- ✅ Confirmer la suppression -->
        <form method="post" action="<%= request.getContextPath() %>/deleteMod">
